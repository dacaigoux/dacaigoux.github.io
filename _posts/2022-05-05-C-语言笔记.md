...
​	字符集分为"源字符集"和"执行字符集"。源字符集是在源代码中的字符集，执行字符集是编译构建完成后，二进制文件中所表示的字符集。
